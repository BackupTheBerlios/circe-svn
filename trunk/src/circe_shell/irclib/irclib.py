            ...
            if command in ["privmsg", "notice"]:
                ...
                for m in messages:
                    if type(m) is types.TupleType:
                        ...
                        m = list(m)
                        if DEBUG:
                            print "command1: %s, source: %s, target: %s, arguments: %s" % (
                                command, prefix, target, m)
                        self._handle_event(Event(command, prefix, target, m))
                    else:
                        if DEBUG:
                            print "command2: %s, source: %s, target: %s, arguments: %s" % (
                                command, prefix, target, [m])
                        self._handle_event(Event(command, prefix, target, [m]))
            else:
                target = None
                ...
                if DEBUG:
                    print "command3: %s, source: %s, target: %s, arguments: %s" % (
...
class DCCConnection(Connection):
    ...
    def process_data(self):
        ...
        for chunk in chunks:
            ...
            arguments = [chunk]
            if DEBUG:
                print "command4: %s, source: %s, target: %s, arguments: %s" % (
                    command, prefix, target, arguments)
            self.irclibobj._handle_event(
                self,
